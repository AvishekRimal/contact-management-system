...
import { 
  PlusCircle, Search, Trash2, Phone, Mail, Briefcase, UserX, Eye, Sparkles, MapPin, ChevronRight, Edit2
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-xl backdrop-blur-xl">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700/80 text-xs text-slate-800 dark:text-slate-200 rounded-xl px-3.5 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition font-semibold"
            value={statusFilter}
            onChange={(e) => {
          ...
          {hasPermission('add_employee') && (
            <Link 
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs px-4 py-2 rounded-xl font-semibold shadow-lg hover:shadow-blue-500/20 transition duration-150"
            >
              <PlusCircle className="h-4 w-4" /> Add Personnel
            </Link>
          )}
        </div>
      </div>
            ...
            {employees.map((emp: any) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Top Right Action Controls (Edit & Delete) */}
                  <div className="absolute top-3 right-3 flex items-center gap-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                    {hasPermission('edit_employee') && (
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-xl transition"
                    ...
                    {hasPermission('delete_employee') && (
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition"
                  ...
                  <div className="flex flex-col items-center pt-2 text-center space-y-3">
                    ...
                    <div className="relative">
                      <div className="w-18 h-18 rounded-2xl bg-slate-100 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 overflow-hidden shadow-md group-hover:scale-105 transition-transform duration-300">
                        <img
                          src={emp.documents?.find((d: any) => d.name === 'Profile Picture')?.url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80'}
                          alt="profile"
                    ...
                    <div className="space-y-1">
                      {emailsList.length > 0 ? (
                        emailsList.map((email: string, idx: number) => (
                          <div key={idx} className="flex items-center gap-2 truncate text-slate-600 dark:text-slate-300" title={email}>
                            <Mail className="h-3.5 w-3.5 shrink-0 text-blue-500 dark:text-blue-400" />
                            <span className="truncate">{email}</span>
                          </div>
                        ))
                      ) : (
                        <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 truncate">
                      ...
                      {mobilesList.length > 0 ? (
                        ...
                        <div className="flex items-center justify-between text-slate-400 dark:text-slate-500">
                          <div className="flex items-center gap-2 truncate">
                            ...
                            <span className="truncate italic">No Mobile</span>
                          </div>
                          {emp.personalInfo?.bloodGroup && (
                            <span className="bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20 text-[10px] font-bold px-1.5 py-0.5 rounded-md shrink-0">
                              {emp.personalInfo.bloodGroup}
