...
import { ArrowLeft, Save, Upload, ShieldAlert, Sparkles, Plus, Trash2, Mail, Phone, Loader2 } from 'lucide-react';
...
export default function EmployeeForm({
  ...
  // For EDIT: pre-fill from initialData.visibleToRoles
  // For CREATE: empty array [] (DO NOT pre-select logged-in user's role!)
  const [selectedRoleIds, setSelectedRoleIds] = useState<string[]>(() => {
    if (mode === 'edit' && initialData?.visibleToRoles) {
      return initialData.visibleToRoles.map((r: any) => (typeof r === 'string' ? r : r._id || r.id));
    }
    return [];
  });
  ...
  const [emailsList, setEmailsList] = useState<string[]>(() => {
    if (mode === 'edit' && initialData?.contactInfo?.emails?.length) {
      return initialData.contactInfo.emails;
    }
    if (mode === 'edit' && initialData?.contactInfo?.email) {
      return [initialData.contactInfo.email];
    }
    return [''];
  });

  const [mobilesList, setMobilesList] = useState<string[]>(() => {
    if (mode === 'edit' && initialData?.contactInfo?.mobiles?.length) {
      return initialData.contactInfo.mobiles;
    }
    if (mode === 'edit' && initialData?.contactInfo?.mobile) {
      return [initialData.contactInfo.mobile];
    }
    return [''];
  });
  ...
  const [form, setForm] = useState({
    ...
    contactInfo: {
      ...
      mobiles: initialData?.contactInfo?.mobiles || [],
      permanentAddress: initialData?.contactInfo?.permanentAddress || '',
      temporaryAddress: initialData?.contactInfo?.temporaryAddress || '',
    },
    personalInfo: {
      birthDate: initialData?.personalInfo?.birthDate || '',
      religion: initialData?.personalInfo?.religion || '',
      maritalStatus: initialData?.personalInfo?.maritalStatus || '',
      gender: initialData?.personalInfo?.gender || '',
      joinDate: initialData?.personalInfo?.joinDate || '',
      bloodGroup: initialData?.personalInfo?.bloodGroup || '',
    },
    officeInfo: {
      department: initialData?.officeInfo?.department || initialData?.department || '',
      courseTaught: initialData?.officeInfo?.courseTaught || '',
      bank: initialData?.officeInfo?.bank || '',
      bankAccount: initialData?.officeInfo?.bankAccount || '',
      panNumber: initialData?.officeInfo?.panNumber || '',
      ssfNumber: initialData?.officeInfo?.ssfNumber || '',
      status: (initialData?.officeInfo?.status || initialData?.status || 'Active') as 'Active' | 'Inactive',
    },
    emergencyContact: {
      contactPerson: initialData?.emergencyContact?.contactPerson || '',
      relation: initialData?.emergencyContact?.relation || '',
      phone: initialData?.emergencyContact?.phone || '',
    },
    documents: (initialData?.documents || []) as Array<{ name: string; url: string; publicId: string; fileType: string }>,
  });

  // Re-sync form state if initialData changes dynamically in edit mode
  useEffect(() => {
    if (mode === 'edit' && initialData) {
      ...
      const roleIds = (initialData.visibleToRoles || []).map((r: any) => (typeof r === 'string' ? r : r._id || r.id));
      setSelectedRoleIds(roleIds);

      const ems = initialData.contactInfo?.emails?.length
        ? initialData.contactInfo.emails
        : (initialData.contactInfo?.email ? [initialData.contactInfo.email] : ['']);
      setEmailsList(ems);

      const mbs = initialData.contactInfo?.mobiles?.length
        ? initialData.contactInfo.mobiles
        : (initialData.contactInfo?.mobile ? [initialData.contactInfo.mobile] : ['']);
      setMobilesList(mbs);
    }
  }, [initialData, mode]);
  ...
  const requiredPermission = mode === 'create' ? 'add_employee' : 'edit_employee';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasPermission(requiredPermission)) {
      toast.error(`Permission Denied: You do not have permission to ${mode === 'create' ? 'add' : 'edit'} personnel records.`);
      return;
    }
    ...
    try {
      const token = getCookie('token');
      const targetUrl = mode === 'create' ? '/api/employees' : `/api/employees/${employeeId}`;
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `Could not ${mode === 'create' ? 'save' : 'update'} profile.`);

      toast.success(
        mode === 'create'
          ? `Personnel profile for "${form.fullName}" registered successfully!`
          : `Personnel profile for "${form.fullName}" updated successfully!`
      );

      if (onSuccess) {
        onSuccess();
      } else {
        router.push(mode === 'create' ? '/dashboard' : `/dashboard/employees/${employeeId}`);
        router.refresh();
      }
    } catch (err: any) {
      toast.error(err.message || `Failed to ${mode === 'create' ? 'create' : 'update'} record`);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="max-w-2xl mx-auto py-16 text-center space-y-4">
        ...
        <p className="text-xs text-slate-400 max-w-md mx-auto">
          You do not have the required (<code className="bg-slate-800 px-1 py-0.5 rounded text-amber-400">{requiredPermission}</code>) permission.
        </p>
        <button
          type="button"
          onClick={onCancel || (() => router.push('/dashboard'))}
          className="inline-flex items-center gap-2 text-xs font-semibold text-blue-400 hover:underline cursor-pointer"
        >
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onCancel || (() => router.push(mode === 'edit' && employeeId ? `/dashboard/employees/${employeeId}` : '/dashboard'))}
          className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition text-slate-700 dark:text-slate-300 shadow-xs cursor-pointer"
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">
            {mode === 'create' ? 'New Personnel Registration' : `Edit Contact Profile: ${form.fullName || 'Personnel Record'}`}
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {mode === 'create'
              ? 'Register employee profiles with complete background data.'
              : 'Update profile details, contact information, address, emergency contact, and visibility access.'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="lg:col-span-8 bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-xl space-y-5 flex flex-col justify-between">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="flex flex-col items-center gap-3 shrink-0">
                ...
                <label className="cursor-pointer bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-white text-[11px] font-semibold px-3 py-1.5 rounded-xl transition border border-slate-300 dark:border-slate-600 flex items-center gap-1.5">
                  {uploading ? (
                    <>
        ...
        <div className="bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-xl space-y-4">
          <h2 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700/60 pb-2">
          ...
          <div className="flex items-center gap-3">
            {onCancel && (
              <button
                ...
                onClick={onCancel}
                disabled={loading}
                className="w-1/3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 font-semibold py-3 px-4 rounded-xl text-xs transition cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
            )}
            <button
              ...
              className={`${onCancel ? 'w-2/3' : 'w-full'} bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-3 px-4 rounded-xl text-xs shadow-lg flex items-center justify-center gap-2 transition cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed`}
            >
              {loading ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <span>{mode === 'create' ? 'Processing Registration...' : 'Saving Modifications...'}</span>
                </>
              ) : (
                <>
                  <Save className="h-4 w-4" />
                  <span>{mode === 'create' ? 'Commit Personnel Record' : 'Save Modifications'}</span>
                </>
              )}
            </button>
          </div>
